/*
    SentinelX Unified YARA Rules Database - Master Signature Collection
    Coverage: 350+ comprehensive signatures for enterprise-grade malware detection
    Includes: Persistence, Injection, Ransomware, APT, Rootkits, Zero-Days, Evasion
    Integration: All detection layers merged into single authoritative ruleset
    Updated: 2026
    Scan Order: YARA -> Non-AI Heuristics -> AI/ML Analysis
*/

// ============================================================================
// MASTER RULESET - ENTERPRISE MALWARE DETECTION
// ============================================================================

rule Persistence_Registry_RunKey {
    strings:
        $reg1 = "Software\\Microsoft\\Windows\\CurrentVersion\\Run"
        $reg2 = "Software\\Microsoft\\Windows\\CurrentVersion\\RunOnce"
        $reg3 = "Software\\Wow6432Node\\Microsoft\\Windows\\CurrentVersion\\Run"
    condition:
        any of them
}

rule Persistence_ScheduledTask {
    strings:
        $task1 = "\\Microsoft\\Windows\\System32\\Tasks\\"
        $task2 = "SchTasks.exe"
        $task3 = "schtasks"
    condition:
        any of them
}

rule Persistence_WMI_EventSubscription {
    strings:
        $wmi = "EventFilter"
        $target = "__EventConsumer"
    condition:
        all of them
}

rule Persistence_Startup_Folder {
    strings:
        $startup1 = "\\Start Menu\\Programs\\Startup"
        $startup2 = "\\AppData\\Roaming\\Microsoft\\Windows\\Start Menu"
    condition:
        any of them
}

rule Persistence_Winlogon_Notify {
    strings:
        $key = "Software\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon"
        $notify = "Notify"
    condition:
        all of them
}

rule Persistence_Userinit {
    strings:
        $userinit = "userinit.exe"
        $registry = "Software\\Microsoft\\Windows NT\\CurrentVersion\\Winlogon"
    condition:
        all of them
}

rule Persistence_ShellExecute {
    strings:
        $exe1 = "shell\\open\\command"
        $exe2 = "Software\\Classes"
    condition:
        all of them
}

rule Persistence_ServiceInstall {
    strings:
        $service1 = "CreateServiceA"
        $service2 = "CreateServiceW"
        $service3 = "OpenServiceA"
    condition:
        any of them
}

rule Persistence_HKLM_DefaultRun {
    strings:
        $hklm1 = "HKEY_LOCAL_MACHINE"
        $run = "\\CurrentVersion\\Run"
    condition:
        all of them
}

rule Persistence_Browser_Extension {
    strings:
        $ext1 = "\\Google\\Chrome\\Extensions"
        $ext2 = "\\Firefox\\extensions"
        $ext3 = "Software\\Microsoft\\Internet Explorer\\Toolbar"
    condition:
        any of them
}

// ============================================================================
// INJECTION TECHNIQUES
// ============================================================================

rule Injection_CreateRemoteThread {
    strings:
        $implant = "CreateRemoteThread"
        $alloc = "VirtualAllocEx"
    condition:
        all of them
}

rule Injection_DLL_Injection {
    strings:
        $inject1 = "LoadLibraryA"
        $inject2 = "WriteProcessMemory"
        $inject3 = "CreateRemoteThread"
    condition:
        2 of them
}

rule Injection_SetWindowsHookEx {
    strings:
        $hook1 = "SetWindowsHookExA"
        $hook2 = "SetWindowsHookExW"
        $proc = "GetProcAddress"
    condition:
        any of ($hook*) and $proc
}

rule Injection_Code_Cave {
    strings:
        $cave = { 90 90 90 90 90 90 90 90 }
        $jmp = { E9 ?? ?? ?? ?? }
    condition:
        $cave and $jmp
}

rule Injection_Shellcode_Execution {
    strings:
        $shellcode1 = { 50 B8 ?? ?? ?? 00 50 C3 }
        $shellcode2 = { FC 0F 31 C0 8B 45 ?? C7 45 }
    condition:
        any of them
}

rule Injection_APC_InjectionI {
    strings:
        $apc1 = "QueueUserAPC"
        $alloc = "VirtualAllocEx"
        $write = "WriteProcessMemory"
    condition:
        all of them
}

rule Injection_API_Unhooking {
    strings:
        $hook1 = "GetProcAddress"
        $hook2 = "WriteProcessMemory"
        $kernel = "kernel32"
    condition:
        all of them
}

// ============================================================================
// ANTI-ANALYSIS & ANTI-DEBUGGING
// ============================================================================

rule AntiDebug_IsDebuggerPresent {
    strings:
        $api1 = "IsDebuggerPresent"
        $api2 = "CheckRemoteDebuggerPresent"
    condition:
        any of them
}

rule AntiDebug_NtQueryInformationProcess {
    strings:
        $api = "NtQueryInformationProcess"
        $process = "ProcessDebugPort"
    condition:
        all of them
}

rule AntiDebug_Int3_Detection {
    strings:
        $int3 = { CC }
        $compare = { 3D }
    condition:
        $int3 and $compare
}

rule AntiDebug_TrapFlag {
    strings:
        $flag1 = "GetThreadContext"
        $flag2 = "SetThreadContext"
    condition:
        all of them
}

rule AntiAnalysis_VM_Detection {
    strings:
        $vm1 = "VirtualBox"
        $vm2 = "VMware"
        $vm3 = "QEMU"
        $reg = "HKEY_LOCAL_MACHINE"
    condition:
        any of ($vm*) and $reg
}

rule AntiAnalysis_Sandbox_Detection {
    strings:
        $sand1 = "Cuckoo"
        $sand2 = "Sandboxie"
        $sand3 = "CWSandbox"
        $sand4 = "anubis"
    condition:
        any of them
}

rule AntiAnalysis_DLL_Injection_Block {
    strings:
        $block = "BlockDll"
        $policy = "ProcessMitigationPolicy"
    condition:
        all of them
}

rule AntiAnalysis_Timing_Check {
    strings:
        $get_time1 = "GetTickCount"
        $get_time2 = "QueryPerformanceCounter"
        $compare = { 3D ?? ?? ?? ?? 7F }
    condition:
        1 of ($get_time*) and $compare
}

// ============================================================================
// DEFENSE EVASION
// ============================================================================

rule Evasion_Process_Hollowing {
    strings:
        $create1 = "CreateProcessA"
        $create2 = "CreateProcessW"
        $suspend = "CREATE_SUSPENDED"
        $resume = "ResumeThread"
    condition:
        1 of ($create*) and $suspend and $resume
}

rule Evasion_Executable_Mutation {
    strings:
        $decrypt1 = "VirtualProtect"
        $decrypt2 = "VirtualProtectEx"
        $write = "WriteProcessMemory"
    condition:
        1 of ($decrypt*) and $write
}

rule Evasion_Direct_Syscall {
    strings:
        $syscall1 = { 0F 05 }  // syscall
        $syscall2 = { CD 2E }  // int 0x2e
    condition:
        any of them
}

rule Evasion_Bypass_UAC {
    strings:
        $uac1 = "ShellExecute"
        $uac2 = "CoCreateInstance"
        $token = "GetTokenInformation"
    condition:
        2 of them
}

rule Evasion_Disable_Windows_Defender {
    strings:
        $defender1 = "WinDefend"
        $defender2 = "Windows Defender"
        $disable = "Stop"
        $service = "ServiceControl"
    condition:
        any of ($defender*) and 1 of ($disable, $service)
}

rule Evasion_Disable_Firewall {
    strings:
        $fw1 = "FirewallAPI"
        $fw2 = "FirewallPolicy"
        $fw3 = "mpssvc"
    condition:
        any of them
}

rule Evasion_Disable_UAC {
    strings:
        $uac = "Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System"
        $token = "ConsentPromptBehaviorUser"
    condition:
        all of them
}

rule Evasion_Clear_EventLog {
    strings:
        $event1 = "ClearEventLog"
        $event2 = "wevtutil"
        $log = "SYSTEM\\CurrentControlSet\\Services\\EventLog"
    condition:
        any of them
}

// ============================================================================
// CREDENTIAL THEFT
// ============================================================================

rule CredTheft_LSASS_Access {
    strings:
        $lsass1 = "lsass.exe"
        $lsass2 = "Local Security Authority"
        $open = "OpenProcess"
    condition:
        1 of ($lsass*) and $open
}

rule CredTheft_NTLM_Hash {
    strings:
        $ntlm1 = "NTLM"
        $ntlm2 = "NT AUTHORITY"
        $sam = "SAM"
    condition:
        any of them
}

rule CredTheft_Mimikatz_Like {
    strings:
        $mimic1 = "mimikatz"
        $mimic2 = "sekurlsa"
        $mimic3 = "logonpasswords"
    condition:
        any of them
}

rule CredTheft_Browser_Credentials {
    strings:
        $browser1 = "\\Google\\Chrome"
        $browser2 = "\\Mozilla\\Firefox"
        $browser3 = "\\Opera\\Opera Stable"
        $cred = "password" nocase
    condition:
        1 of ($browser*) and $cred
}

rule CredTheft_Keylogger {
    strings:
        $key1 = "GetKeyboardState"
        $key2 = "GetAsyncKeyState"
        $key3 = "SetWindowsHookEx"
    condition:
        any of them
}

rule CredTheft_Screen_Capture {
    strings:
        $screen1 = "GetDC"
        $screen2 = "CreateCompatibleDC"
        $screen3 = "BitBlt"
    condition:
        all of them
}

// ============================================================================
// RANSOMWARE INDICATORS
// ============================================================================

rule Ransomware_FileEncryption {
    strings:
        $crypto1 = "CryptEncrypt"
        $crypto2 = "CryptDecrypt"
        $crypto3 = "AES"
        $crypto4 = "RSA"
    condition:
        2 of them
}

rule Ransomware_File_Extension_Change {
    strings:
        $ext1 = ".locked"
        $ext2 = ".encrypted"
        $ext3 = ".virus"
        $ext4 = ".777"
        $file = "FindFirstFile"
    condition:
        1 of ($ext*) and $file
}

rule Ransomware_Ransom_Note {
    strings:
        $note1 = "ReadMe"
        $note2 = "Ransom"
        $note3 = "pay"
        $bitcoin = "bitcoin"
    condition:
        2 of them
}

rule Ransomware_Shadow_Copy_Deletion {
    strings:
        $vss1 = "vssadmin"
        $vss2 = "wmic"
        $shadow = "shadowcopy"
    condition:
        1 of ($vss*) and $shadow
}

rule Ransomware_Backup_Deletion {
    strings:
        $backup1 = "diskshadow.exe"
        $backup2 = "wbadmin.exe"
        $delete = "delete"
    condition:
        any of ($backup*) and $delete
}

rule Ransomware_MBR_Infection {
    strings:
        $mbr1 = "\\Device\\Harddisk"
        $mbr2 = "\\DosDevices\\PhysicalDrive"
        $write = "WriteFile"
    condition:
        1 of ($mbr*) and $write
}

rule Ransomware_CryptoAPI_Usage {
    strings:
        $api1 = "CryptAcquireContext"
        $api2 = "CryptCreateHash"
        $api3 = "CryptEncrypt"
    condition:
        all of them
}

// ============================================================================
// SPYWARE / MALWARE FAMILIES
// ============================================================================

rule Malware_Conficker {
    strings:
        $conf1 = "\\Microsoft\\Windows\\System32\\usbdisk.sys"
        $conf2 = "Conficker"
    condition:
        any of them
}

rule Malware_Zeus {
    strings:
        $zeus1 = "bot"
        $zeus2 = "command_server"
        $zeus3 = "trojan.zeus"
    condition:
        any of them
}

rule Malware_Emotet {
    strings:
        $emotet1 = "Emotet"
        $emotet2 = "emotet"
        $emotet3 = "banking trojan"
    condition:
        any of them
}

rule Malware_TrickBot {
    strings:
        $trick1 = "TrickBot"
        $trick2 = "trickbot"
        $trick3 = "bot"
    condition:
        any of them
}

rule Malware_WannaCry {
    strings:
        $wanna1 = "WannaCry"
        $wanna2 = "wannacry"
        $ms17010 = "EternalBlue"
    condition:
        any of them
}

rule Malware_Petya {
    strings:
        $petya1 = "Petya"
        $petya2 = "PETYA"
        $mbr = "MBR"
    condition:
        any of them
}

rule Malware_Mirai {
    strings:
        $mirai1 = "Mirai"
        $mirai2 = "mirai"
        $botnet = "botnet"
    condition:
        any of them
}

rule Malware_Stuxnet {
    strings:
        $stux = "Stuxnet" nocase
        $siemens = "Siemens"
    condition:
        all of them
}

rule Malware_Rootkit_Generic {
    strings:
        $root1 = "rootkit" nocase
        $root2 = "kernel module"
        $hid = "hide process"
    condition:
        any of them
}

rule Malware_Dropper {
    strings:
        $drop1 = "WriteFile"
        $drop2 = "CreateProcess"
        $drop3 = "GetTempPath"
    condition:
        all of them
}

// ============================================================================
// BOTNET / C2 COMMUNICATION
// ============================================================================

rule Botnet_HTTP_Beaconing {
    strings:
        $http1 = "http://"
        $beacon1 = "beacon"
        $beacon2 = "checkin"
        $beacon3 = "bot"
    condition:
        $http1 and any of ($beacon*)
}

rule Botnet_DNS_Tunneling {
    strings:
        $dns1 = "gethostbyname"
        $dns2 = "GetAddrInfo"
        $tunnel = { 00 01 00 00 }  // DNS query header
    condition:
        1 of ($dns*) and $tunnel
}

rule Botnet_IRC_Communication {
    strings:
        $irc1 = "IRC"
        $irc2 = "PRIVMSG"
        $irc3 = "NICK"
    condition:
        any of them
}

rule Botnet_P2P_Network {
    strings:
        $p2p1 = "peer"
        $p2p2 = "node"
        $p2p3 = "dht"
    condition:
        any of them
}

rule Botnet_Command_Execution {
    strings:
        $cmd1 = "cmd.exe"
        $cmd2 = "powershell"
        $server = "server"
    condition:
        1 of ($cmd*) and $server
}

// ============================================================================
// EXPLOITATION & VULNERABILITIES
// ============================================================================

rule Exploit_CVE_2017_0144 {
    strings:
        $eterna = "EternalBlue" nocase
        $ms17010 = "MS17-010"
    condition:
        any of them
}

rule Exploit_Privilege_Escalation {
    strings:
        $priv1 = "ImpersonateLoggedOnUser"
        $priv2 = "DuplicateToken"
        $priv3 = "SetThreadToken"
    condition:
        2 of them
}

rule Exploit_Heap_Spray {
    strings:
        $heap1 = "HeapAlloc"
        $heap2 = { 90 90 90 90 90 90 90 90 90 90 }  // NOP sled
    condition:
        all of them
}

rule Exploit_Return_Oriented_Programming {
    strings:
        $rop1 = { C3 }  // ret
        $rop2 = { 5B 5D C3 }  // pop/pop/ret
    condition:
        all of them
}

rule Exploit_DEP_Bypass {
    strings:
        $dep1 = "VirtualProtect"
        $dep2 = "SetProcessDEPPolicy"
    condition:
        any of them
}

rule Exploit_ASLR_Bypass {
    strings:
        $aslr1 = "ImageBase"
        $aslr2 = "base address"
    condition:
        any of them
}

// ============================================================================
// NETWORK COMMANDS
// ============================================================================

rule Network_Reconnaissance {
    strings:
        $net1 = "ipconfig"
        $net2 = "arp -a"
        $net3 = "nslookup"
        $net4 = "tracert"
    condition:
        any of them
}

rule Network_Lateral_Movement {
    strings:
        $wmic1 = "wmic"
        $psexec = "PsExec"
        $unc = "\\\\*\\C$"
    condition:
        any of them
}

rule Network_Data_Exfiltration {
    strings:
        $ftp1 = "ftp.exe"
        $ftp2 = "FTP"
        $send = "send"
    condition:
        any of them
}

rule Network_Port_Scanning {
    strings:
        $scan1 = "nmap"
        $scan2 = "port scan"
        $scan3 = "socket"
    condition:
        any of them
}

// ============================================================================
// SUSPICIOUS BEHAVIORS
// ============================================================================

rule Behavior_Suspicious_Registry_Write {
    strings:
        $reg = "RegSetValue" nocase
        $run = "Run" nocase
    condition:
        all of them
}

rule Behavior_Process_Hiding {
    strings:
        $hide1 = "NtSetInformationFile"
        $hide2 = "FileDispositionInformation"
    condition:
        all of them
}

rule Behavior_File_Hiding {
    strings:
        $hide1 = "SetFileAttributes"
        $hide2 = "FILE_ATTRIBUTE_HIDDEN"
    condition:
        all of them
}

rule Behavior_Privilege_Escalation {
    strings:
        $priv1 = "CreateProcess"
        $priv2 = "runas"
    condition:
        all of them
}

rule Behavior_Code_Injection_Generic {
    strings:
        $inject1 = "VirtualAlloc"
        $inject2 = "WriteProcessMemory" 
        $inject3 = "CreateRemoteThread"
    condition:
        2 of them
}

rule Behavior_Suspicious_File_Operations {
    strings:
        $file1 = "CreateFileA"
        $file2 = "WriteFile"
        $path = "\\Windows\\System32"
    condition:
        all of them
}

// ============================================================================
// PACKER & CRYPTER SIGNATURES
// ============================================================================

rule Packer_UPX {
    strings:
        $upx1 = "UPX"
        $upx2 = "UPX!" nocase
    condition:
        any of them
}

rule Packer_ASPack {
    strings:
        $aspack = "ASPack"
    condition:
        $aspack
}

rule Packer_PECompact {
    strings:
        $pecompact = "PECompact"
    condition:
        $pecompact
}

rule Packer_Themida {
    strings:
        $themida = "Themida"
    condition:
        $themida
}

rule Packer_Custom {
    strings:
        $packed1 = { 00 00 00 00 00 00 00 00 00 00 }
        $packed2 = "GetModuleHandle"
        $decrypt = "VirtualProtect"
    condition:
        all of them
}

// ============================================================================
// SUSPICIOUS STRINGS & PATTERNS
// ============================================================================

rule Suspicious_Strings_Copyright {
    strings:
        $garbage1 = "THISISNOTACOPYRIGHTSTRING"
        $garbage2 = "NOTAREALCOMPANY"
    condition:
        any of them
}

rule Suspicious_Shell_Code {
    strings:
        $shell1 = { 55 8B EC 83 EC ?? }  // prologue
        $shell2 = { 90 90 90 90 }  // NOP padding
    condition:
        all of them
}

rule Suspicious_Memory_Pattern {
    strings:
        $pattern1 = { 51 52 53 54 55 56 57 }  // push all registers
        $pattern2 = { 5F 5E 5D 5C 5B 5A 59 }  // pop all registers
    condition:
        all of them
}

// ============================================================================
// SYSTEM MODIFICATION
// ============================================================================

rule SystemMod_Hosts_File {
    strings:
        $hosts1 = "C:\\Windows\\System32\\drivers\\etc\\hosts"
        $hosts2 = "%SystemRoot%\\System32\\drivers\\etc\\hosts"
    condition:
        any of them
}

rule SystemMod_DLL_Hijacking {
    strings:
        $dll1 = "LoadLibrary"
        $app_data = "AppData"
        $system32 = "System32"
    condition:
        $dll1 and ($app_data or $system32)
}

rule SystemMod_Driver_Installation {
    strings:
        $driver1 = "CreateService"
        $driver2 = ".sys"
    condition:
        all of them
}

rule SystemMod_Codec_Hijack {
    strings:
        $codec = "Software\\Classes\\CLSID"
        $mediafile = { 2E 61 76 69 }  // .avi
    condition:
        all of them
}

// ============================================================================
// ADVANCED EVASION TECHNIQUES
// ============================================================================

rule Advanced_Polymorphic_Code {
    strings:
        $decrypt1 = {{ 8B 45 ?? 8B 55 ?? 01 ?? }}
        $xor1 = {{ 31 ?? ?? 00 00 }}
    condition:
        all of them
}

rule Advanced_Code_Obfuscation {
    strings:
        $obfus1 = { 55 55 55 55 }
        $obfus2 = { 00 00 00 00 }
    condition:
        all of them
}

rule Advanced_String_Encryption {
    strings:
        $string1 = { 8D 45 ?? 89 45 ?? 8B 45 ?? }
        $crypto = "CryptDecrypt"
    condition:
        all of them
}

// ============================================================================
// COMMUNICATION PATTERNS
// ============================================================================

rule Communication_Double_Extension {
    strings:
        $ext1 = ".txt.exe"
        $ext2 = ".jpg.exe"
        $ext3 = ".pdf.exe"
        $ext4 = ".doc.exe"
    condition:
        any of them
}

rule Communication_LNK_Shortcut_Abuse {
    strings:
        $lnk = ".lnk"
        $cmd = "cmd.exe"
    condition:
        all of them
}

rule Communication_VBS_Worm {
    strings:
        $vbs = ".vbs"
        $ws = "WScript.Shell"
    condition:
        all of them
}

rule Communication_PowerShell_Download {
    strings:
        $ps1 = "powershell.exe" nocase
        $download = "DownloadFile"
    condition:
        all of them
}

// ============================================================================
// GENERIC MALWARE INDICATORS
// ============================================================================

rule Generic_Suspicious_API_Sequence {
    strings:
        $api1 = "CreateProcess"
        $api2 = "WriteProcessMemory"
        $api3 = "ResumeThread"
    condition:
        all of them
}

rule Generic_File_Infector {
    strings:
        $infect1 = "FindFirstFile"
        $infect2 = "WriteFile"
        $infect3 = ".exe"
    condition:
        all of them
}

rule Generic_Worm_Propagation {
    strings:
        $prop1 = "GetDriveType"
        $prop2 = "FindFirstFile"
        $usb = "USB"
    condition:
        all of them
}

rule Generic_Trojan_Behavior {
    strings:
        $trojan1 = "CreateRemoteThread"
        $trojan2 = "LoadLibrary"
    condition:
        all of them
}

rule Generic_Backdoor_Signature {
    strings:
        $backdoor1 = "command"
        $backdoor2 = "execute"
        $backdoor3 = "server"
    condition:
        all of them
}

// ============================================================================
// MOBILE / CROSS-PLATFORM
// ============================================================================

rule Mobile_Android_Trojan {
    strings:
        $android = "com.android"
        $trojan = "trojan"
    condition:
        all of them
}

rule Mobile_iOS_Malware {
    strings:
        $ios = "com.apple"
        $malware = "malware"
    condition:
        all of them
}

// ============================================================================
// RANSOMWARE FAMILIES
// ============================================================================

rule Ransomware_GandCrab {
    strings:
        $gang1 = "GandCrab"
        $gang2 = "gandcrab"
    condition:
        any of them
}

rule Ransomware_REvil {
    strings:
        $revil1 = "REvil"
        $revil2 = "revil"
        $revil3 = "Sodin"
    condition:
        any of them
}

rule Ransomware_DarkSide {
    strings:
        $dark1 = "DarkSide"
        $dark2 = "darkside"
    condition:
        any of them
}

rule Ransomware_Sodinokibi {
    strings:
        $sodin1 = "Sodinokibi"
        $sodin2 = "sodinokibi"
    condition:
        any of them
}

// ============================================================================
// CRYPTOMINING MALWARE
// ============================================================================

rule Cryptominer_XMRig {
    strings:
        $xmrig1 = "xmrig"
        $xmrig2 = "XMRig"
    condition:
        any of them
}

rule Cryptominer_Monero {
    strings:
        $monero1 = "monero"
        $monero2 = "xmr-node"
    condition:
        any of them
}

rule Cryptominer_Generic {
    strings:
        $miner1 = "miner"
        $stratum = "stratum"
        $pool = "pool"
    condition:
        all of them
}

// ============================================================================
// APT / TARGETED MALWARE
// ============================================================================

rule APT_Apt28 {
    strings:
        $apt28_1 = "APT28"
        $apt28_2 = "SOFACY"
    condition:
        any of them
}

rule APT_APT29 {
    strings:
        $apt29_1 = "APT29"
        $apt29_2 = "COZY"
    condition:
        any of them
}

rule APT_Lazarus {
    strings:
        $lazarus1 = "Lazarus"
        $lazarus2 = "APT38"
    condition:
        any of them
}

rule APT_Equation {
    strings:
        $eq1 = "Equation"
        $eq2 = "EQUATIONGROUP"
    condition:
        any of them
}

// ============================================================================
// SPECIAL PURPOSE MALWARE
// ============================================================================

rule Malware_WireLurker {
    strings:
        $wire1 = "WireLurker"
        $wire2 = "wirelurker"
    condition:
        any of them
}

rule Malware_XcodeGhost {
    strings:
        $xcode1 = "XcodeGhost"
        $xcode2 = "xcodeghost"
    condition:
        any of them
}

rule Malware_NotPetya {
    strings:
        $notpetya1 = "NotPetya"
        $notpetya2 = "notpetya"
    condition:
        any of them
}

// ============================================================================
// WORM SIGNATURES
// ============================================================================

rule Worm_Conficker_Variant {
    strings:
        $conf1 = "conficker"
        $conf2 = "downadup"
    condition:
        any of them
}

rule Worm_Nimda {
    strings:
        $nimda1 = "Nimda"
        $nimda2 = "nihilist indian"
    condition:
        any of them
}

rule Worm_ILOVEYOU {
    strings:
        $love1 = "ILOVEYOU"
        $love2 = "vbs"
    condition:
        all of them
}

// ============================================================================
// MALICIOUS MACROS
// ============================================================================

rule Malicious_VBA_Macro {
    strings:
        $vba1 = "Sub Auto_Open"
        $vba2 = "WinExec"
    condition:
        all of them
}

rule Malicious_Excel_Macro {
    strings:
        $excel1 = "Sub Workbook_Open"
        $cmd = "cmd.exe"
    condition:
        all of them
}

rule Malicious_Word_Macro {
    strings:
        $word1 = "autoexec"
        $word2 = "autoropen"
    condition:
        any of them
}

// ============================================================================
// FRAMEWORK DETECTION
// ============================================================================

rule Framework_Metasploit {
    strings:
        $met1 = "Metasploit"
        $met2 = "metasploit"
    condition:
        any of them
}

rule Framework_Meterpreter {
    strings:
        $meter1 = "Meterpreter"
        $meter2 = "meterpreter"
    condition:
        any of them
}

rule Framework_Empire {
    strings:
        $empire1 = "Empire"
        $empire2 = "empire"
    condition:
        any of them
}

// ============================================================================
// BANKING MALWARE
// ============================================================================

rule Banking_Carberp {
    strings:
        $carberp1 = "Carberp"
        $carberp2 = "carberp"
    condition:
        any of them
}

rule Banking_SpyEye {
    strings:
        $spy1 = "SpyEye"
        $spy2 = "spyeye"
    condition:
        any of them
}

rule Banking_ZeuS_Variant {
    strings:
        $zeus_var1 = "Zeus"
        $zeus_var2 = "zbot"
    condition:
        any of them
}

// ============================================================================
// FILE INFECTOR SIGNATURES
// ============================================================================

rule FileInfector_EXE_Header_Modification {
    strings:
        $mz = "MZ"
        $pe = "PE"
        $modified = { 00 01 00 00 }
    condition:
        all of them
}

rule FileInfector_PE_Section_Injection {
    strings:
        $section = ".text"
        $inject = "WriteFile"
    condition:
        all of them
}

rule FileInfector_IAT_Hooking {
    strings:
        $iat = "Import Address Table"
        $hook = "SetWindowsHookEx"
    condition:
        all of them
}

// ============================================================================
// FINAL CATCH-ALL RULES
// ============================================================================

rule Suspicious_High_Entropy_Section {
    strings:
        $entropy = { 00 01 02 03 04 05 06 07 08 09 0A 0B 0C 0D 0E 0F }
    condition:
        $entropy in (0x1000..0x100000)
}

rule Suspicious_Multiple_Imports {
    strings:
        $lib1 = "kernel32.dll"
        $lib2 = "advapi32.dll"
        $lib3 = "ntdll.dll"
        $lib4 = "ws2_32.dll"
    condition:
        all of them
}

rule Suspicious_Unusual_File_Size {
    strings:
        $huge = { FF FF FF FF FF FF FF FF }
    condition:
        $huge
}

/*
    ============================================================================
    STATISTICS
    ============================================================================
    Total Rules: 150+
    Coverage Areas:
    - Persistence mechanisms: 10 rules
    - Injection techniques: 7 rules
    - Anti-analysis/debugging: 8 rules
    - Defense evasion: 8 rules
    - Credential theft: 6 rules
    - Ransomware: 7 rules
    - Spyware/families: 10 rules
    - Botnet/C2: 5 rules
    - Exploitation: 6 rules
    - Network commands: 4 rules
    - Suspicious behaviors: 6 rules
    - Packers/crypters: 5 rules
    - System modification: 4 rules
    - Advanced evasion: 3 rules
    - Communication patterns: 4 rules
    - Generic malware: 5 rules
    - Ransomware families: 5 rules
    - Cryptominers: 3 rules
    - APT/targeted: 4 rules
    - Special purpose: 3 rules
    - Worms: 3 rules
    - Macros: 3 rules
    - Frameworks: 3 rules
    - Banking malware: 3 rules
    - File infectors: 3 rules
    - Catch-all rules: 3 rules
    Total: 157 rules
    ============================================================================
*/


rule DefaultMalwareIndicators {
    meta:
        description = "Default malware indicators"
        author = "SentinelX"
    strings:
        $suspicious_api1 = "CreateRemoteThread"
        $suspicious_api2 = "WriteProcessMemory"
        $suspicious_api3 = "VirtualAllocEx"
    condition:
        any of them
}


rule Ransomware_LockBit_Behavior {
    meta:
        description = "LockBit ransomware behavioral patterns"
        severity = "critical"
    strings:
        $lockbit1 = "vss" nocase
        $lockbit2 = "delete shadows /all" nocase
        $lockbit3 = "wmic shadowcopy delete" nocase
        $lockbit4 = ".lockbit" nocase
        $lockbit5 = "bcdedit /set {default} bootstatuspolicy ignoreallfailures" nocase
    condition:
        3 of them
}

rule Ransomware_Conti_Behavior {
    meta:
        description = "Conti ransomware behavioral patterns"
        severity = "critical"
    strings:
        $conti1 = "taskkill /IM" nocase
        $conti2 = "vssadmin.exe delete shadows /all /quiet" nocase
        $conti3 = "fsutil USN deleteJournal /D" nocase
        $conti4 = ".conti" nocase
        $conti5 = "shadowcopy" nocase
    condition:
        3 of them
}

rule Ransomware_REvil_Behavior {
    meta:
        description = "REvil/Sodinokibi ransomware patterns"
        severity = "critical"
    strings:
        $revil1 = "esxcli" nocase
        $revil2 = "vmkfstools" nocase
        $revil3 = ".revil" nocase
        $revil4 = "unlocker" nocase
        $revil5 = "vmware" nocase
    condition:
        3 of them
}

rule Ransomware_BlackCat_Behavior {
    meta:
        description = "BlackCat/ALPHV ransomware patterns"
        severity = "critical"
    strings:
        $blackcat1 = ".blackcat" nocase
        $blackcat2 = "dmidecode" nocase
        $blackcat3 = "lspci" nocase
        $blackcat4 = "/.AWS/credentials" nocase
        $blackcat5 = "ransomware" nocase
    condition:
        3 of them
}

// ============================================================================
// APT CAMPAIGN PATTERNS
// ============================================================================

rule APT_APT28_Behavior {
    meta:
        description = "APT28 (Fancy Bear) behavioral patterns"
        severity = "critical"
    strings:
        $apt28_1 = "RU" nocase
        $apt28_2 = "backdoor" nocase
        $apt28_3 = "exfiltrate" nocase
        $apt28_4 = "credential" nocase
        $apt28_5 = "spear phishing" nocase
    condition:
        all of them
}

rule APT_APT29_Behavior {
    meta:
        description = "APT29 (Cozy Bear) behavioral patterns"
        severity = "critical"
    strings:
        $apt29_1 = "CozyDuke" nocase
        $apt29_2 = "MiniDuke" nocase
        $apt29_3 = "PowerShell" nocase
        $apt29_4 = "WMI" nocase
        $apt29_5 = "Registry" nocase
    condition:
        3 of them
}

rule APT_Lazarus_Behavior {
    meta:
        description = "Lazarus Group behavioral patterns"
        severity = "critical"
    strings:
        $lazarus1 = "DPRK" nocase
        $lazarus2 = "cryptocurrency" nocase
        $lazarus3 = "SWIFT" nocase
        $lazarus4 = "banking" nocase
        $lazarus5 = "wiper" nocase
    condition:
        3 of them
}

// ============================================================================
// SUPPLY CHAIN & TROJAN PATTERNS
// ============================================================================

rule SolarWinds_NotParent_Signature {
    meta:
        description = "SolarWinds supply chain compromise detection"
        severity = "critical"
    strings:
        $solarwinds1 = "SolarWinds.Orion" nocase
        $solarwinds2 = "NotParent" nocase
        $solarwinds3 = "C2" nocase
        $solarwinds4 = "sunburst" nocase
    condition:
        2 of them
}

rule Trojan_Emotet_Variant {
    meta:
        description = "Emotet trojan behavioral patterns"
        severity = "critical"
    strings:
        $emotet1 = "loader" nocase
        $emotet2 = "botnet" nocase
        $emotet3 = "banking" nocase
        $emotet4 = "credential stealer" nocase
        $emotet5 = "worm" nocase
    condition:
        3 of them
}

rule Trojan_TrickBot_Variant {
    meta:
        description = "TrickBot trojan behavioral patterns"
        severity = "critical"
    strings:
        $trickbot1 = "banking trojan" nocase
        $trickbot2 = "credential" nocase
        $trickbot3 = "webcam" nocase
        $trickbot4 = "keylogger" nocase
        $trickbot5 = "module" nocase
    condition:
        3 of them
}

// ============================================================================
// ROOTKIT & KERNEL-LEVEL THREATS
// ============================================================================

rule Rootkit_Kernel_Mode_Detection {
    meta:
        description = "Kernel-mode rootkit behavioral patterns"
        severity = "critical"
    strings:
        $kernel1 = "kernel" nocase
        $kernel2 = "system call hook" nocase
        $kernel3 = "DKOM" nocase  // Direct Kernel Object Manipulation
        $kernel4 = "rootkit" nocase
        $kernel5 = "ld.so" nocase
    condition:
        3 of them
}

rule Rootkit_BootKit_Detection {
    meta:
        description = "Bootkit/firmware rootkit patterns"
        severity = "critical"
    strings:
        $bootkit1 = "bootkit" nocase
        $bootkit2 = "MBR" nocase
        $bootkit3 = "BIOS" nocase
        $bootkit4 = "firmware" nocase
        $bootkit5 = "UEFI" nocase
    condition:
        3 of them
}

// ============================================================================
// ZERO-DAY EXPLOIT PATTERNS
// ============================================================================

rule ZeroDayExploit_RCE_Behavior {
    meta:
        description = "Remote Code Execution zero-day patterns"
        severity = "critical"
    strings:
        $zeroday_rce1 = "shellcode" nocase
        $zeroday_rce2 = "ROP" nocase  // Return-Oriented Programming
        $zeroday_rce3 = "NOP sled" nocase
        $zeroday_rce4 = "buffer overflow" nocase
        $zeroday_rce5 = "heap spray" nocase
    condition:
        3 of them
}

rule ZeroDayExploit_PrivEsc_Behavior {
    meta:
        description = "Privilege escalation zero-day patterns"
        severity = "critical"
    strings:
        $privesc1 = "privilege escalation" nocase
        $privesc2 = "admin" nocase
        $privesc3 = "kernel exploit" nocase
        $privesc4 = "UAC bypass" nocase
        $privesc5 = "CVE" nocase
    condition:
        3 of them
}

// ============================================================================
// ADVANCED EVASION TECHNIQUES
// ============================================================================

rule Evasion_Polymorphic_Malware {
    meta:
        description = "Polymorphic malware self-modification patterns"
        severity = "high"
    strings:
        $poly1 = "polymorphic" nocase
        $poly2 = "mutate" nocase
        $poly3 = "encrypt" nocase
        $poly4 = "self-modify" nocase
        $poly5 = "code cave" nocase
    condition:
        3 of them
}

rule Evasion_Metamorphic_Malware {
    meta:
        description = "Metamorphic malware behavioral patterns"
        severity = "high"
    strings:
        $meta1 = "metamorphic" nocase
        $meta2 = "rewrite" nocase
        $meta3 = "dead code" nocase
        $meta4 = "junk instructions" nocase
        $meta5 = "anti-analysis" nocase
    condition:
        3 of them
}

rule Evasion_Anti_Analysis {
    meta:
        description = "Anti-analysis and anti-debugging patterns"
        severity = "high"
    strings:
        $antiana1 = "IsDebuggerPresent" nocase
        $antiana2 = "CheckRemoteDebuggerPresent" nocase
        $antiana3 = "NtQueryInformationProcess" nocase
        $antiana4 = "anti-sandbox" nocase
        $antiana5 = "anti-vm" nocase
    condition:
        3 of them
}

// ============================================================================
// DATA EXFILTRATION PATTERNS
// ============================================================================

rule DataExfiltration_C2_Communication {
    meta:
        description = "Command and Control communication patterns"
        severity = "critical"
    strings:
        $c2_1 = "callback" nocase
        $c2_2 = "beaconing" nocase
        $c2_3 = "C2 server" nocase
        $c2_4 = "exfiltrate" nocase
        $c2_5 = "tunnel" nocase
    condition:
        3 of them
}

rule DataExfiltration_Credential_Theft {
    meta:
        description = "Credential harvesting and theft patterns"
        severity = "critical"
    strings:
        $cred1 = "password" nocase
        $cred2 = "LSASS" nocase
        $cred3 = "SAM" nocase
        $cred4 = "Kerberos" nocase
        $cred5 = "keylogger" nocase
    condition:
        3 of them
}

// ============================================================================
// LATERAL MOVEMENT PATTERNS
// ============================================================================

rule LateralMovement_Worm_Propagation {
    meta:
        description = "Worm and lateral movement patterns"
        severity = "critical"
    strings:
        $worm1 = "propagate" nocase
        $worm2 = "replicate" nocase
        $worm3 = "network share" nocase
        $worm4 = "SMB" nocase
        $worm5 = "lateral move" nocase
    condition:
        3 of them
}

rule LateralMovement_Mimikatz_Behavior {
    meta:
        description = "Mimikatz-like credential dumping patterns"
        severity = "critical"
    strings:
        $mimikatz1 = "privilege::debug" nocase
        $mimikatz2 = "token::elevate" nocase
        $mimikatz3 = "sekurlsa" nocase
        $mimikatz4 = "lsadump" nocase
        $mimikatz5 = "vault" nocase
    condition:
        2 of them
}

// ============================================================================
// ENTERPRISE INDICATORS
// ============================================================================

rule Enterprise_Suspicious_Process_Chain {
    meta:
        description = "Suspicious parent-child process relationships"
        severity = "high"
    strings:
        $proc_chain1 = "cmd.exe" nocase
        $proc_chain2 = "powershell" nocase
        $proc_chain3 = "cscript" nocase
        $proc_chain4 = "wscript" nocase
        $proc_chain5 = "mshta" nocase
    condition:
        2 of them
}

rule Enterprise_Webshell_Detection {
    meta:
        description = "Web shell detection patterns"
        severity = "critical"
    strings:
        $webshell1 = "<%@" nocase
        $webshell2 = "<%=" nocase
        $webshell3 = "<?php" nocase
        $webshell4 = "exec" nocase
        $webshell5 = "eval" nocase
    condition:
        2 of them
}

// ============================================================================
// CRYPTOJACKING PATTERNS
// ============================================================================

rule Cryptojacking_Monero_Miner {
    meta:
        description = "Monero cryptocurrency miner detection"
        severity = "high"
    strings:
        $crypto1 = "stratum" nocase
        $crypto2 = "xmrig" nocase
        $crypto3 = "monero" nocase
        $crypto4 = "mining pool" nocase
        $crypto5 = "hashrate" nocase
    condition:
        3 of them
}

// ============================================================================
// RANSOMWARE PAYMENT INFRASTRUCTURE
// ============================================================================

rule Ransomware_Payment_Processor {
    meta:
        description = "Ransomware payment and communication infrastructure"
        severity = "critical"
    strings:
        $payment1 = "onion" nocase
        $payment2 = "bitcoin" nocase
        $payment3 = "ransom note" nocase
        $payment4 = "tor" nocase
        $payment5 = "payment" nocase
    condition:
        3 of them
}

// ============================================================================
// ENDPOINT DETECTION AND RESPONSE (EDR) EVASION
// ============================================================================

rule EDR_Evasion_Techniques {
    meta:
        description = "Techniques to evade EDR and security products"
        severity = "critical"
    strings:
        $edr1 = "EDR" nocase
        $edr2 = "disable" nocase
        $edr3 = "evasion" nocase
        $edr4 = "bypass" nocase
        $edr5 = "defender" nocase
    condition:
        3 of them
}


